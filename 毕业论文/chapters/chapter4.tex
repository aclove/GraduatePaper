...
令$S=\{G|G \in Imp(\mathcal{G}) \land W^{G}_M = W^{\mathcal{G}}_M\}$为
...
\begin{example}
\label{ex:6}
表~\ref{tab4.1}的第一列显示了图~\ref{fig2.1:subfiga}中的不确定图的蕴含图边集，
该不确定图一共有三颗最小生成树
...
\begin{table}
\centering
\caption{图~\ref{fig2.1:subfiga}的可靠性评估表.}\label{tab4.1}
\begin{tabular}{lll}
\toprule
蕴含图边集 &包含的最小生成树 &出现概率\\
\midrule
...
因此，由引理~\ref{lm:4.2.2}可以得到改节点中所有可靠蕴含图的存在概率之和。
算法~\ref{alg:imst}为不确定图中最小生成树的可靠性求解算法。
...
\begin{experiment}
\label{ex:4.1}
首先我们针对不确定图为完全图且所有边的权值都相等时的数据进行实验，
其所有的生成树都为最小生成树。
...
从图$~\ref{fig4.1}$可以得出算法$IERM$的时间复杂度与顶点数无关，
...
\subfigure[$n = 100$]{
    ...
    \centering
    \includegraphics[width=\figwidth]{Fig4-1a}
    \end{minipage}}
\subfigure[$n = 200$]{
    \label{fig4.1:subfigb}
    \begin{minipage}[t]{\figwidth}
    \centering
    \includegraphics[width=\figwidth]{Fig4-1b}
    \end{minipage}}
\subfigure[$n = 300$]{
    \label{fig4.1:subfigc}
    \begin{minipage}[t]{\figwidth}
    \centering
    \includegraphics[width=\figwidth]{Fig4-1c}
    \end{minipage}}
\subfigure[$n = 400$]{
    \label{fig4.1:subfigd}
    \begin{minipage}[t]{\figwidth}
    \centering
    \includegraphics[width=\figwidth]{Fig4-1d}
    \end{minipage}}
