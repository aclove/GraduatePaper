...
边集$E_O \subseteq E$且$E_O \cap E_T = \varnothing$；
...
选取前$K$个最小生成树，
即可得到Top-K最优生成树。
算法~\ref{alg:top-k1}为TOP-K最优生成树算法，
...
\begin{algorithm}[!htb]
\caption{TOP-K最优生成树算法}
\label{alg:top-k1}
\begin{algorithmic}[1]
\Statex
\setcounter{ALG@line}{0}
\Algphase{\textbf{模块1：}TOP-K最优生成树算法}
...
\begin{proof}
显然，每次调用函数$FindAllMST$时，
需要调用一次函数$GetMST$并且产生$n-1$次的递归调用。
这样，函数$FindAllMST$最多被调用$Nn+1 \approx Nn$次。
然而，函数$GetMST$求解最小生成树的时间复杂度为$O(m)$。
...
若$E_{Diff}=\{e_1,e_2, \cdots, e_k\}$表示这两颗生成树中不同边的集合，
...
在生成树$T'$中的路径$P^u_v$为$u(u_0) \to u_1, \cdots, u_{x-1}, \to v(u_x)$。
...
\end{equation}
则算法~\ref{alg:ns}对子树中的节点进行编号，
显然我们可以调用函数$\Call{SetNumber}{T,1,1}$为生成树$T$中的所有顶点进行编号。
\begin{algorithm}[!htb]
\caption{子树编号算法}
\label{alg:ns}
\begin{algorithmic}[1]
...
设$RT_i$为顶点$i$每次被处理后记录的最近公共祖先，
显然对于每个在路径$P^{u_1}_{u_2}$中的顶点$w$，
满足$RT_w = v$。
设函数$\Call{GetFather}{T,x}$返回顶点$x$的父节点。
则算法~\ref{alg:swap:edge}为替换边求解算法。
...
且$e_3, e_4 \in V$。
由于$w(e_3) < w(e_4)$，
所以我们首先查找顶点4和顶点5之间的路径，
由于$w(e_1)=w(e_3)$，
所以边$e_3$为边$e_1$的替换边。
显然顶点2为顶点4和顶点5的最近公共祖先，
我们用顶点2来替换路径$P^4_5$，
则$RT_4=2$且$RT_5=2$。
之后我们再处理边$e_4$，
我们知道顶点5已经被其最近公共祖先替换，
...
\begin{figure}[htb]
\centering
\includegraphics[width=\imagewidth]{Fig3-4}
\caption{路径压缩过程。}
\label{fig3.4}
\end{figure}
...
\begin{algorithm}[htb]
\caption{TOP-K最优生成树算法-并查集优化。}
\label{alg:top-k2}
\begin{algorithmic}[1]
\Statex
\setcounter{ALG@line}{0}
\Algphase{\textbf{模块1：}TOP-K最优生成树算法-并查集优化}
\Procedure{TopKMST}{$K, \mathcal{G}$} \Comment $\mathcal{G}=(V,E,W,P)$
...
由于使用了路径压缩，生成树中的每条边则只会被访问一次，
所以函数$\Call{FindSwapEdge}{}$的时间复杂度为$O(m)$(假设$m > n$)；
...
算法~\ref{alg:top-k3}为使用启发式搜索的TOP-K最优生成树算法，
$PriorQue$按照传入的第一个参数进行从大到小排序，
$elem.T$为以$elem.E_T$为边集的最小生成树。

\begin{algorithm}[!htb]
\caption{TOP-K最优生成树算法-启发式搜索优化。}
\label{alg:top-k3}
\begin{algorithmic}[1]
