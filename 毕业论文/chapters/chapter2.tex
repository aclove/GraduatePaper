...
\begin{example}
\label{ex:1}
图~\ref{fig2.1:subfiga}为一个不确定图$\mathcal{G}_{1}$，
边上的两个数字分别代表权值和概率，
由于该不确定图有五条可能出现的边，因此该不确定图有$2^5$个蕴含图。
图\ref{fig2.1:subfigb}为不确定图$\mathcal{G}_{1}$的一个蕴含图，
...
显然，最优生成树不是唯一的，我们也可以将最优生成树看作不确定图中的最小生成树中的最大乘积生成树。
...
因此可以假设不确定图$\mathcal{G} = (V,E,W,P)$中所有的边按照如下优先级进行排序：
...
\begin{example}
\label{ex:2}
图~\ref{fig2.2}为一个不确定图$\mathcal{G}$，按照优先级排序后的边集为$\{e_1,e_2, \cdots, e_{10}\}$。
图~\ref{fig2.3}为图~\ref{fig2.2}中不确定图的最优生成树求解过程，
红色标记的边为加入生成树中的边，
...
\label{fig2.2}
\centering
\includegraphics[width=\imagewidth]{Fig2-2}
\caption{例~\ref{ex:2}：
不确定图$\mathcal{G}$。}
\end{figure}
...
\label{fig2.3}
\centering
\includegraphics[width=\textwidth]{Fig2-3}
\caption{例~\ref{ex:2}：
最优生成树计算过程。}
\end{figure}
...
\begin{enumerate}
 ...
 \item 假设新加入的边为$e=(u,v)$，寻找生成树$T_O^{\mathcal{G}}$中从顶点$u$到顶点$v$的路径中优先级最高的边并删除。这样可以得到一颗新的生成树；
 \item 重复前面2步，知道所有的边都已经处理，这样我们能够得到$|E|-|V|+1$颗新的生成树，从这些生成树中求出最优的生成树则是不确定图$\mathcal{G}$的次优生成树。
\end{enumerate}

在第二步中，我们需要知道最优生成树中任意两个顶点之间优先级最高的边，我们可以在求解最优生成树时进行预处理得到：
\begin{enumerate}
 \item 设$F(i,j)$为在最优生成树中从顶点$i$到顶点$j$的路径中优先级最高的边；
 \item 在求解最优生成树时，假设边$e=(u,v)$加入了边集$E_O^{\mathcal{G}}$，则对于图中所有顶点$x$，应该进行如下转换：
 \begin{enumerate}
    \item 如果边$e$比边$F(x,u)$优先级更高，则$F(x,u) = e$；
    \item 如果边$e$比边$F(x,v)$优先级更高，则$F(x,v) = e$；
 \end{enumerate}
\end{enumerate}
